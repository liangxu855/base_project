package com.lujiaozhuang.client.utils.http.compress;
...
import com.lujiaozhuang.client.code.MyApplication;
import com.lujiaozhuang.client.utils.http.httpquest.FileParam;
...
import static com.lujiaozhuang.client.utils.http.compress.Preconditions.checkNotNull;
...
public class Luban {
    ...
    private static String DEFAULT_DISK_CACHE_DIR = "ImgCache";
    private static String DEFAULT_DISK_CACHE_FILE_HEAD = "lubanCache";


    private OnCompressListener compressListener;
    private ArrayList<FileParam> mFiles = new ArrayList<>();
    private ArrayList<FileParam> compressFiles = new ArrayList<FileParam>();
    ...
    private static File getPhotoCacheDir(Context context, String cacheName) {
        File cacheDir = new File(MyApplication.appSDCachePath);
        if (cacheDir != null) {
    ...
    public Luban launch() {
        ...
        Observable.create(new Observable.OnSubscribe<Integer>() {
            ...
            public void call(Subscriber<? super Integer> subscriber) {
                compressFiles.clear();
                for (FileParam f : mFiles) {

                    FileParam ff = compress(f);
                    if (ff != null) {
                        compressFiles.add(ff);
                ...
                .subscribe(new Action1<Integer>() {
                    ...
                    public void call(Integer progress) {
                        if (compressListener != null) compressListener.onLoading(progress,mFiles.size());
                    }
                });
    ...
    public Luban load(FileParam... file) {
        mFiles.clear();
        for (FileParam f : file) {
            mFiles.add(f);
        }
        return this;
    }

    public Luban load(List<FileParam> file) {
        mFiles.clear();
        mFiles.addAll(file);
        return this;
    }
    ...
    private FileParam compress(@NonNull FileParam file) {
        try {
            File ff = file.getFile();
            ...
            FileParam fp = new FileParam(ff);
            fp.setKey(file.getKey());
            fp.setFileName(file.getFile().getName());
            return fp;
        } catch (Exception e) {
            return null;
        }
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = MyApplication.appSDCachePath + File.separator + DEFAULT_DISK_CACHE_FILE_HEAD + System.currentTimeMillis()+".jpg";
        double size;
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String filePath = file.getAbsolutePath();
        String thumbFilePath = MyApplication.appSDCachePath + File.separator + DEFAULT_DISK_CACHE_FILE_HEAD + System.currentTimeMillis();
     ...
     */
    public File compress(String largeImagePath, String thumbFilePath, int width, int height, int angle, long size) {
